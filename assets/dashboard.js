(() => {
  ...
  async function addFocus(anchor, news){
    ...
    sec.innerHTML=`<div class="dash-head"><h2>DANAS U FOKUSU</h2><p>Četiri najnovije vijesti s fotografijom</p></div><div class="focus-grid">${items.map(n=>`<a class="focus-card" href="${esc(newsLink(n))}"><img src="${esc(n.image)}" alt=""><div><p>${esc(n.category||'VIJESTI')} · ${esc(n.source||'')}</p><h3>${esc(n.title)}</h3></div></a>`).join('')}</div>`;
  ...
  async function renderHomeDashboard(){
    ...
    const install=document.createElement('section'); install.id='pwaInstallBox'; install.className='pwa-install'; install.innerHTML='<h2>Dodaj WEB VIJESTI na početni zaslon</h2><p>Otvori portal kao aplikaciju na mobitelu ili računalu.</p><button id="pwaInstallBtn">DODAJ</button>';
