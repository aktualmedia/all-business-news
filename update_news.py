#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""WEB VIJESTI update: RSS/Atom + odabrani HTML izvori.
...
Napomena: kod agregiranih vijesti koristi se naslov, izvorni sažetak/opis ako postoji,
fotografija ako je dostupna u zapisu, odnosno tehnička naslovna zamjenska fotografija
kada izvor ne isporučuje sliku kroz RSS/HTML.
"""
...
DATA = ROOT / "data"
TIMEOUT = 14
UA = "WEB-VIJESTI-Aktual-Media/7.7 (+https://aktualmedia.github.io/all-business-news/)"
MAX_PER_CATEGORY = 1000
MAX_ITEMS_PER_FEED = 12
MAX_HTML_ITEMS_PER_SOURCE = 18
GALLERY_LIMIT = 500
...
BLOCKED_TITLE_WORDS = ("politika", "politič", "politički", "izbor", "vlada", "stranka", "sport", "sportski", "tenis", "nogomet", "wta", "utakmica")
SKIP_PATH_WORDS = ("/politika", "/sport", "/tag/politika", "/tag/sport", "facebook.com", "instagram.com", "tiktok.com", "youtube.com", "linkedin.com", "twitter.com", "x.com", "privacy", "privatnost", "cookies", "kolac", "kontakt", "oglasavanje", "impressum")

EXTRA_FEEDS = [
    {"id": "extra-bloomberg-hr-001", "category": "poslovanje", "name": "Bloomberg Hrvatska - poslovanje", "url": "https://news.google.com/rss/search?q=site:bloomberg.com%20Croatia%20business&hl=hr&gl=HR&ceid=HR:hr", "enabled": True},
    {"id": "extra-bloomberg-hr-002", "category": "ekonomija", "name": "Bloomberg Hrvatska - ekonomija", "url": "https://news.google.com/rss/search?q=site:bloomberg.com%20Croatia%20economy&hl=hr&gl=HR&ceid=HR:hr", "enabled": True},
    {"id": "extra-bloomberg-hr-003", "category": "financije", "name": "Bloomberg Hrvatska - financije", "url": "https://news.google.com/rss/search?q=site:bloomberg.com%20Croatia%20finance&hl=hr&gl=HR&ceid=HR:hr", "enabled": True},
    {"id": "extra-bloomberg-hr-004", "category": "trzista", "name": "Bloomberg Hrvatska - tržišta", "url": "https://news.google.com/rss/search?q=site:bloomberg.com%20Croatia%20markets&hl=hr&gl=HR&ceid=HR:hr", "enabled": True},

    {"id": "extra-symbol-kultura", "kind": "html", "category": "kultura", "name": "Symbol Quorum - kultura i umjetnost", "url": "https://symbol-quorum.com/category/kultura-umjetnost/", "enabled": True},
    {"id": "extra-symbol-poduzetnistvo", "kind": "html", "category": "poslovanje", "name": "Symbol Quorum - poduzetništvo", "url": "https://symbol-quorum.com/category/poduzetnistvo/", "enabled": True},
    {"id": "extra-symbol-gastronomija", "kind": "html", "category": "pica", "name": "Symbol Quorum - gastronomija", "url": "https://symbol-quorum.com/category/gastronomija/", "enabled": True},
    {"id": "extra-symbol-lifestyle", "kind": "html", "category": "lifestyle", "name": "Symbol Quorum - lifestyle", "url": "https://symbol-quorum.com/category/lifestyle/", "enabled": True},

    {"id": "extra-journal-moda", "kind": "html", "category": "lifestyle", "name": "Journal.hr - moda", "url": "https://www.journal.hr/moda/", "enabled": True, "allow_hosts": ["specijal.journal.hr"]},
    {"id": "extra-journal-ljepota", "kind": "html", "category": "lifestyle", "name": "Journal.hr - ljepota", "url": "https://www.journal.hr/ljepota/", "enabled": True, "allow_hosts": ["specijal.journal.hr"]},
    {"id": "extra-journal-lifestyle", "kind": "html", "category": "lifestyle", "name": "Journal.hr - lifestyle", "url": "https://www.journal.hr/lifestyle/", "enabled": True, "allow_hosts": ["specijal.journal.hr"]},
    {"id": "extra-journal-kultura", "kind": "html", "category": "kultura", "name": "Journal.hr - kultura", "url": "https://www.journal.hr/kultura/", "enabled": True, "allow_hosts": ["specijal.journal.hr"]},
    {"id": "extra-journal-interijeri", "kind": "html", "category": "dizajn", "name": "Journal.hr - interijeri", "url": "https://www.journal.hr/lifestyle/interijeri/", "enabled": True, "allow_hosts": ["specijal.journal.hr"]},
    {"id": "extra-journal-gastro", "kind": "html", "category": "hedonizam", "name": "Journal.hr - gastro", "url": "https://www.journal.hr/lifestyle/gastro/", "enabled": True, "allow_hosts": ["specijal.journal.hr"]},
...
def placeholder_image(source, title, category="vijesti"):
    seed = hashlib.sha1(f"{source}|{title}|{category}".encode("utf-8", "ignore")).hexdigest()[:12]
    return f"https://picsum.photos/seed/wv-news-{seed}/1200/750"
...
def html_image(*texts):
    ...
    for pat in patterns:
        ...
        if m:
            ...
            if "," in val:
                val = val.split(",", 1)[0].strip().split(" ", 1)[0]
            return val
    return ""
...
def child_text(el, names):
    ...
    for ch in list(el):
        if ch.tag.split("}")[-1].lower() in wanted:
...
def child_attr(el, names, attr):
    wanted = {n.lower() for n in names}
    for ch in list(el):
        if ch.tag.split("}")[-1].lower() in wanted and ch.attrib.get(attr):
            return ch.attrib.get(attr)
    return ""


def allowed_html_link(base_url, href, feed):
    if not href or href.startswith(("#", "mailto:", "tel:", "javascript:")):
        return ""
    link = urllib.parse.urljoin(base_url, html.unescape(href))
    low = link.lower()
    if any(w in low for w in SKIP_PATH_WORDS):
        return ""
    base_host = urllib.parse.urlsplit(base_url).netloc.lower().replace("www.", "")
    host = urllib.parse.urlsplit(link).netloc.lower().replace("www.", "")
    allowed = {str(h).lower().replace("www.", "") for h in feed.get("allow_hosts", [])}
    if host and host != base_host and host not in allowed:
        return ""
    return link


def parse_html_source(html_bytes, feed):
    raw = html_bytes.decode("utf-8", "ignore")
    out, seen = [], set()
    base = feed.get("url") or ""
    for m in re.finditer(r'<a\b[^>]*href=["\']([^"\']+)["\'][^>]*>(.*?)</a>', raw, flags=re.I | re.S):
        link = allowed_html_link(base, m.group(1), feed)
        if not link:
            continue
        title = textify(m.group(2))
        title = re.sub(r"^(Vijesti|Trendovi|Gastro|Moda|Kultura|Lifestyle|Interijeri|Ljepota)\s+", "", title, flags=re.I).strip()
        if len(title) < 24 or len(title) > 210:
            continue
        low_title = title.lower()
        if any(w in low_title for w in BLOCKED_TITLE_WORDS):
            continue
        ...
        img = html_image(m.group(2), around)
        if img:
            img = urllib.parse.urljoin(base, img)
        else:
            img = placeholder_image(feed.get("name"), title, feed.get("category"))
        out.append({
            "id": stable_id(link, title),
            "title": title,
            "description": f"Izvorni naslov/sažetak s portala {feed.get('name')}. Nastavak se čita kod izvornog izdavača.",
            "url": link,
            "image": img,
            "source": feed.get("name") or feed.get("id") or "HTML",
        ...
        if len(out) >= MAX_HTML_ITEMS_PER_SOURCE:
            break
    return out
...
def parse_feed(xml_bytes, feed):
    ...
    for el in entries[:MAX_ITEMS_PER_FEED]:
        ...
        link = child_text(el, ["link"]) or child_attr(el, ["link"], "href")
        desc = child_text(el, ["description", "summary", "content", "encoded"])
        pub = child_text(el, ["pubDate", "published", "updated", "date"])
        img = attr_image(el) or html_image(desc)
        if not title or not link:
            continue
        title = textify(title)
        if any(w in title.lower() for w in BLOCKED_TITLE_WORDS):
            continue
        if not img:
            img = placeholder_image(feed.get("name"), title, feed.get("category"))
        out.append({"id": stable_id(link, title), "title": title, "description": textify(desc)[:900], "url": link.strip(), "image": img.strip(), "source": feed.get("name") or feed.get("id") or "RSS", "source_id": feed.get("id"), "category": feed.get("category") or "vijesti", "published_at": parse_date(pub) or datetime.now(timezone.utc).isoformat(), "fetched_at": datetime.now(timezone.utc).isoformat()})
    return out


def parse_source(raw, feed):
    if str(feed.get("kind") or "").lower() == "html":
        return parse_html_source(raw, feed)
...
def normalize_item(n):
    ...
    if not n["title"] or not n["url"]:
        return None
    if any(w in n["title"].lower() for w in BLOCKED_TITLE_WORDS):
        return None
    ...
    if not n.get("image"):
        n["image"] = placeholder_image(n.get("source"), n.get("title"), n.get("category"))
...
def build_gallery(news_items):
    ...
    while len(gallery) < GALLERY_LIMIT:
        cat = themes[(i - 1) % len(themes)]
        img = f"https://picsum.photos/seed/wv-{cat}-{i:03d}/1400/900"
        if img not in seen:
            seen.add(img)
            gallery.append({"title": f"{CATEGORY_LABELS.get(cat, cat.upper())} · fotografija {i:03d}", "description": "Tematska fotografija za WEB VIJESTI galeriju", "image": img, "category": cat, "source": "Lorem Picsum public photo stream", "source_url": "https://picsum.photos/", "created_at": datetime.now(timezone.utc).isoformat(), "auto_gallery": True})
        i += 1
...
def main():
    ...
    ok = failed = 0
    for feed in feeds:
        row = {"id": feed.get("id"), "name": feed.get("name"), "category": feed.get("category"), "url": feed.get("url"), "kind": feed.get("kind", "rss"), "count": 0, "status": "failed"}
        try:
            items = parse_source(fetch(feed["url"]), feed)
            found.extend(items)
            row["count"] = len(items)
    ...
    title_seen = {c: set() for c in CATEGORIES}
    for n in merged:
        cat = n.get("category") or "vijesti"
        tk = norm_title(n.get("title"))
        if tk in title_seen[cat] or len(by_cat[cat]) >= MAX_PER_CATEGORY:
            continue
        title_seen[cat].add(tk)
        by_cat[cat].append(n)
    final = []
    for c in CATEGORIES:
